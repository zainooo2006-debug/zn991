...
export type LivePayload = { kind: "theme"; json: unknown } | { kind: "reload" } | null;
  ...
  return (
    <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] overflow-hidden flex flex-col h-full">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-[var(--color-border)] flex-wrap">
        ...
        <span
          className={`text-[10px] px-2 py-0.5 rounded ${ready ? "bg-green-500/15 text-green-600" : "bg-amber-500/15 text-amber-600"}`}
        >
        ...
        <input
          ...
          className="flex-1 min-w-[120px] px-2 py-1 rounded bg-[var(--color-bg)] border border-[var(--color-border)] text-xs"
          dir="ltr"
        />
        <button
          onClick={reload}
