...
export function WarrantyOverview() {
  ...
  useEffect(() => {
    call({ data: undefined as never })
      .then((r) => {
        setStats(r.stats);
        setLatest(r.latest as never);
      })
      .catch((e) => {
        setErr(e instanceof Error ? e.message : "تعذر التحميل");
        setStats({ customers: 0, warranties: 0, active: 0, expired: 0 });
      });
  }, [call]);
    ...
    <div className="space-y-4">
      {err && (
        <div className="p-3 rounded-lg bg-amber-50 text-amber-800 border border-amber-200 text-sm">
          يرجى تسجيل الدخول من{" "}
          <a href="/warranty/auth" className="underline font-bold">
            بوابة الضمانات
          </a>{" "}
          بحساب المسؤول لعرض بيانات الضمانات. ({err})
        </div>
      )}
          ...
          {latest.map((r) => {
            ...
            return (
              ...
              >
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs border ${statusColor[s]}`}>
                  {statusLabel[s]}
                </span>
              </div>
            );
...
export function WarrantiesTab() {
  ...
  async function extend(id: string, current: string) {
    ...
    const e = await mutFn({
      data: { op: "warranty_extend", id, expiry_date: d.toISOString().slice(0, 10) },
    });
    if (e) alert(e);
    else load();
  }
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <div className="relative flex-1 min-w-48">
          ...
          <input
            ...
            placeholder="بحث برقم/عميل/جوال/هيكل..."
        ...
        <button
          onClick={load}
          className="p-2 rounded-lg bg-slate-100 dark:bg-slate-700 hover:bg-slate-200"
        >
                ...
                return (
                  <tr key={r.id} className="border-t border-slate-100 dark:border-slate-700">
                    ...
                    <td className="p-2">
                      ...
                      <div className="text-xs text-slate-500">{r.customers?.phone}</div>
                    </td>
                    <td className="p-2 text-xs">
                      {r.warranty_brands?.name ?? "-"} / {r.film_types?.name ?? "-"}
                    </td>
                    <td className="p-2 text-xs">{formatDateAr(r.activation_date)}</td>
                    <td className="p-2 text-xs">{formatDateAr(r.expiry_date)}</td>
                    <td className="p-2">
                      <span className={`text-xs px-2 py-0.5 rounded-full border ${statusColor[s]}`}>
                        {statusLabel[s]}
                      </span>
                    </td>
                    <td className="p-2 whitespace-nowrap">
                      ...
                      <button
                        onClick={() => extend(r.id, r.expiry_date)}
...
export function WarrantyCustomersTab() {
  ...
  async function addManual() {
    ...
    const e = await mutFn({
      data: { op: "customer_insert", full_name: name.trim(), phone: phone.trim() },
    });
    if (e) alert(e);
    else load();
  }
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-900 text-xs">
              <tr>
                ...
                <th className="p-2 text-right">التسجيل</th>
                <th className="p-2"></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id} className="border-t border-slate-100 dark:border-slate-700">
                  ...
                  <td className="p-2 whitespace-nowrap">
                    <button
                      onClick={() => edit(r)}
                      className="text-xs px-2 py-1 rounded bg-blue-100 text-blue-700 mr-1"
                    >
                      تعديل
                    </button>
                    <button
...
export function WarrantySimpleCrud({
  ...
  const [rows, setRows] = useState<
    Array<Record<string, unknown> & { id: string; is_active?: boolean }>
  >([]);
  const [busy, setBusy] = useState(false);
  const load = async () => {
    setBusy(true);
  ...
  async function toggle(r: { id: string; is_active?: boolean }) {
    const e = await mutFn({
      data: { op: "simple_toggle", table, id: r.id, is_active: !r.is_active },
    });
    if (e) alert(e);
    else load();
  }
        ...
        {busy ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-900 text-xs">
              <tr>
                {fields.map((f) => (
                  <th key={f.k} className="p-2 text-right">
                    {f.l}
                  </th>
                ))}
                <th className="p-2 text-right">الحالة</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-slate-100 dark:border-slate-700">
                  ...
                  <td className="p-2">
                    <button
                      onClick={() => toggle(r)}
                      className={`text-xs px-2 py-0.5 rounded-full border ${r.is_active ? "bg-green-100 text-green-800 border-green-300" : "bg-gray-100 text-gray-700 border-gray-300"}`}
                    >
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={fields.length + 2} className="p-8 text-center text-slate-500">
                    لا توجد بيانات
                  </td>
                </tr>
              )}
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 px-4 py-2 rounded-lg bg-amber-500 text-white hover:bg-amber-600 text-sm font-bold"
        >
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-1 flex-wrap">
                        <select
                          ...
                          onChange={(e) => {
                            if (e.target.value) {
                              doGrant(u.id, e.target.value as AppRole);
                              e.target.value = "";
                            }
                          }}
      ...
      >
        <div className="flex items-center justify-between">
          <h3 className="font-black text-lg flex items-center gap-2">
            <Shield className="w-5 h-5 text-amber-500" /> مستخدم جديد
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex gap-2 pt-2">
          <button
            type="submit"
            disabled={busy}
            className="flex-1 py-2 rounded-lg bg-amber-500 text-white font-bold hover:bg-amber-600 disabled:opacity-60"
          >
