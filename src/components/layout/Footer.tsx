...
const wallets = ["جيب", "موبايل موني", "ون كاش", "كريمي حاسب", "محفظتي", "جوالي", "فلوسك"];
    ...
    <footer className="bg-white border-t border-[var(--color-hairline)] mt-16">
      <div className="max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 md:grid-cols-4 gap-10">
        <div>
          ...
          <p className="text-xs text-[var(--color-ink-soft)] mt-3">
            المدير العام:{" "}
            <span className="font-semibold text-[var(--color-ink)]">{c.managerName}</span>
          </p>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-[var(--color-ink-soft)]">
            <li>
              <Link to="/shop" className="hover:text-[var(--color-gold)]">
                المتجر
              </Link>
            </li>
            <li>
              <Link to="/services" className="hover:text-[var(--color-gold)]">
                الخدمات
              </Link>
            </li>
            <li>
              <Link to="/offers" className="hover:text-[var(--color-gold)]">
                العروض
              </Link>
            </li>
            <li>
              <Link to="/vip" className="hover:text-[var(--color-gold)]">
                VIP
              </Link>
            </li>
            <li>
              <Link to="/about" className="hover:text-[var(--color-gold)]">
                من نحن
              </Link>
            </li>
            <li>
              <Link to="/contact" className="hover:text-[var(--color-gold)]">
                اتصل بنا
              </Link>
            </li>
            <li>
              <Link to="/track" className="hover:text-[var(--color-gold)]">
                تتبع الطلب
              </Link>
            </li>
          </ul>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-[var(--color-ink-soft)]">
            ...
            <li className="flex items-center gap-2">
              ...
              <a href={whatsappLink("مرحباً")}>{WHATSAPP_NUMBER.replace("967", "")}</a>
            </li>
            <li className="flex items-start gap-2">
              <MapPin className="w-4 h-4 text-[var(--color-gold)] mt-0.5" /> {c.address}
            </li>
            <li className="flex items-start gap-2">
              <Clock className="w-4 h-4 text-[var(--color-gold)] mt-0.5" /> {c.hours}
            </li>
          </ul>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {wallets.map((w) => (
              <span
                key={w}
                className="text-xs bg-[var(--color-surface)] border border-[var(--color-hairline)] rounded-full px-3 py-1 text-[var(--color-ink-soft)]"
              >
                {w}
              </span>
            ))}
      ...
      <div className="border-t border-[var(--color-hairline)]">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row gap-2 justify-between items-center text-xs text-[var(--color-ink-soft)]">
          ...
          <p>
            تطوير:{" "}
            <a href={`tel:${DEV_PHONE}`} className="text-[var(--color-gold)] font-semibold">
              صديق الزين {DEV_PHONE}
            </a>
          </p>
        </div>
      </div>
