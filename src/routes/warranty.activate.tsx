  ...
  useEffect(() => {
    ...
    (async () => {
      const [b, f, br, c] = await Promise.all([
        ...
        supabase
          .from("film_types")
          .select("id, name, warranty_months, brand_id")
          .eq("is_active", true)
          .order("sort_order"),
      ...
      if (c.data) {
        ...
        const carsRes = await supabase
          .from("cars" as never)
          .select("id, brand_id, model, year, plate_number, vin")
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6 border border-slate-200 dark:border-slate-700">
          ...
          <div className="mb-5 p-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/50">
            <label className="block">
              <div className="text-xs font-bold text-amber-800 dark:text-amber-300 mb-2 flex items-center gap-1">
                <Car className="w-4 h-4" /> اختر سيارة من قائمتك
              </div>
              <select
                value={carId}
                onChange={(e) => setCarId(e.target.value)}
                className="w-full px-3 py-2 border border-amber-300 dark:border-amber-700 rounded-lg bg-white dark:bg-slate-900"
              >
                ...
                {cars.map((c) => {
                  ...
                  return (
                    <option key={c.id} value={c.id}>
                      {[b, c.model, c.year, c.plate_number].filter(Boolean).join(" · ")}
                    </option>
                  );
                })}
              </select>
        ...
        {cars.length === 0 && (
          <div className="mb-5 p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50 text-sm text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
            ...
            مسبقًا لتوفير الوقت في المرات القادمة.
          </div>
        )}

        <form onSubmit={submit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <TxtField
            label="رقم الضمان (اختياري - يُولّد تلقائيًا)"
            value={warrantyNumber}
            onChange={setWarrantyNumber}
          ...
          <Select
            ...
            value={brandId}
            onChange={setBrandId}
            options={[{ v: "", l: "-- اختر --" }, ...brands.map((b) => ({ v: b.id, l: b.name }))]}
          />
          <Select
            label="نوع اللاصق"
            value={filmId}
            ...
            options={[
              { v: "", l: "-- اختر --" },
              ...films.map((f) => ({ v: f.id, l: `${f.name} (${f.warranty_months} شهر)` })),
            ]}
          />
          <TxtField
            label="رقم الهيكل / التسلسلي"
            value={vin}
            onChange={setVin}
            placeholder="VIN..."
          />
          ...
          <Select
            ...
            options={[{ v: "", l: "-- اختر --" }, ...branches.map((b) => ({ v: b.id, l: b.name }))]}
          />

          {msg && (
            <div
              className={`sm:col-span-2 text-sm p-3 rounded-lg ${msg.t === "err" ? "bg-red-50 text-red-700 border border-red-200" : "bg-green-50 text-green-700 border border-green-200"}`}
            >
              {msg.m}
            </div>
          )}
