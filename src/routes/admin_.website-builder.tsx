...
export const Route = createFileRoute("/admin_/website-builder")({
  head: () => ({
    meta: [{ title: "Website Builder — لوحة التحكم" }, { name: "robots", content: "noindex" }],
  }),
      ...
      <Shell>
        <div className="max-w-md mx-auto p-8 text-center">
          ...
          <p className="text-sm text-[var(--color-ink-soft)] mb-4">
            هذه الصفحة متاحة للمدراء فقط. سجّل الدخول بحساب مدير.
          </p>
          <Link to="/warranty/auth" className="btn-primary">
            تسجيل الدخول
          </Link>
        </div>
      </Shell>
  ...
  return (
    <Shell>
      <div className="max-w-[1600px] mx-auto px-4 py-6">
        <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
          <div>
            ...
            <p className="text-sm text-[var(--color-ink-soft)]">
              محرك بناء مواقع ديناميكي مدفوع بـ JSON مع معاينة مباشرة
            </p>
          </div>
        ...
        <div
          className={
            previewOpen ? "grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-4" : ""
          }
        >
          <div className="min-w-0">
            ...
            {tab === "pages" && (
              <TablePanel
                table="website_pages"
                title="Pages"
                jsonFields={["page_json", "seo_json"]}
                textFields={[
                  { k: "name", l: "Name" },
                  { k: "slug", l: "Slug" },
                  { k: "status", l: "Status (draft/published)" },
                ]}
                onEditPath={(r) => setPreviewPath("/" + (r.slug ?? ""))}
              />
            )}
            {tab === "components" && (
              <TablePanel
                ...
                title="Components"
                jsonFields={["schema_json", "settings_json"]}
                textFields={[
                  { k: "name", l: "Name" },
            ...
            {tab === "layouts" && (
              <TablePanel
                table="website_layouts"
                title="Layouts"
                jsonFields={["layout_json"]}
                textFields={[{ k: "name", l: "Name" }]}
              />
            )}
            {tab === "templates" && (
              <TablePanel
                ...
                title="Templates"
                jsonFields={["template_json"]}
                textFields={[
                  { k: "name", l: "Name" },
                  { k: "type", l: "Type (section/page)" },
                  { k: "category", l: "Category" },
                ]}
              />
            )}
            ...
            {tab === "menus" && (
              <TablePanel
                table="website_menus"
                title="Menus"
                jsonFields={["menu_json"]}
                textFields={[
                  { k: "name", l: "Name" },
                  { k: "location", l: "Location (header/footer)" },
                ]}
              />
            )}
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-bold">
          {title}{" "}
          <span className="text-sm text-[var(--color-ink-soft)]">({q.data?.length ?? 0})</span>
        </h2>
        <div className="flex gap-2">
          <button onClick={() => qc.invalidateQueries({ queryKey: key })} className="btn-outline">
            <RefreshCw className="w-4 h-4" />
          </button>
          <button onClick={newRow} className="btn-primary">
      ...
      {editing ? (
        ...
        <div className="space-y-2">
          {(q.data ?? []).map((r: any) => (
            <div
              key={r.id}
              className="bg-[var(--color-surface)] rounded-lg p-3 flex items-center justify-between gap-3"
            >
          ...
          {!q.isLoading && (q.data ?? []).length === 0 && (
            <div className="text-center py-8 text-sm text-[var(--color-ink-soft)]">
              لا توجد عناصر. اضغط "جديد" لإضافة أول عنصر.
            </div>
          )}
        </div>
      )}
...
function RowEditor({
  ...
  const [values, setValues] = useState<any>(() => {
    ...
    jsonFields.forEach((f) => {
      v[f] = JSON.stringify(row[f] ?? {}, null, 2);
    });
    return v;
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      /* invalid JSON — skip until user fixes it */
    }
  ...
  const submit = async () => {
    ...
    try {
      ...
      for (const f of jsonFields) {
        try {
          out[f] = JSON.parse(values[f] || "{}");
        } catch {
          throw new Error(`JSON غير صحيح في الحقل: ${f}`);
        }
      }
...
function MediaPanel() {
  ...
  const qc = useQueryClient();
  const q = useQuery({
    queryKey: ["wb", "website_media"],
    queryFn: () => list({ data: { table: "website_media" as any } }),
  });
        ...
        {(q.data ?? []).map((m: any) => (
          <div key={m.id} className="bg-[var(--color-surface)] rounded-lg overflow-hidden">
            <div className="aspect-square bg-black/10">
              {/^image\//.test(m.metadata?.type ?? "") ||
              /\.(png|jpe?g|webp|gif|svg)$/i.test(m.file_url) ? (
                <img src={m.file_url} className="w-full h-full object-cover" alt="" />
              ) : (
                <div className="w-full h-full flex items-center justify-center text-xs text-[var(--color-ink-soft)]">
                  ملف
                </div>
              )}
            </div>
...
function SettingsPanel() {
  ...
  const qc = useQueryClient();
  const q = useQuery({
    queryKey: ["wb", "website_settings"],
    queryFn: () => list({ data: { table: "website_settings" as any } }),
  });
  ...
  useEffect(() => {
    if (row) setTxt(JSON.stringify(row.settings_json ?? {}, null, 2));
  }, [row?.id]);
  if (!row)
    return (
      <div className="text-center py-6 text-sm text-[var(--color-ink-soft)]">جاري التحميل...</div>
    );
  return (
    <div className="bg-[var(--color-surface)] rounded-xl p-4 space-y-3">
      ...
      <button
        onClick={async () => {
          ...
          try {
            ...
            await upsert({
              data: {
                table: "website_settings" as any,
                values: { id: row.id, settings_json: parsed },
              },
            });
...
function SEOPanel() {
  ...
  const qc = useQueryClient();
  const q = useQuery({
    queryKey: ["wb", "website_pages"],
    queryFn: () => list({ data: { table: "website_pages" as any } }),
  });
  const [editing, setEditing] = useState<any | null>(null);
  const [txt, setTxt] = useState("");
  useEffect(() => {
    if (editing) setTxt(JSON.stringify(editing.seo_json ?? {}, null, 2));
  }, [editing?.id]);
  return (
    <div>
      ...
      {editing ? (
        <div className="bg-[var(--color-surface)] rounded-xl p-4 space-y-3">
          <div className="text-xs">
            Page: <b>{editing.name}</b> ({editing.slug})
          </div>
          <textarea
            value={txt}
            onChange={(e) => setTxt(e.target.value)}
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  await upsert({
                    data: {
                      table: "website_pages" as any,
                      values: { id: editing.id, seo_json: parsed },
                    },
                  });
                  setEditing(null);
        ...
        <div className="space-y-2">
          {(q.data ?? []).map((p: any) => (
            <div
              key={p.id}
              className="bg-[var(--color-surface)] rounded-lg p-3 flex justify-between items-center"
            >
              <div className="text-sm">
                <b>{p.name}</b> · <span className="text-[var(--color-ink-soft)]">/{p.slug}</span>
              </div>
              <button onClick={() => setEditing(p)} className="btn-outline text-xs">
                تعديل SEO
              </button>
            </div>
          ))}
          {(q.data ?? []).length === 0 && (
            <div className="text-center py-6 text-sm text-[var(--color-ink-soft)]">
              أضف صفحات أولاً من تبويب Pages.
            </div>
          )}
        </div>
      )}
...
function CustomCodePanel() {
  ...
  useEffect(() => {
    if (row) {
      setHtml(row.custom_html ?? "");
      setCss(row.custom_css ?? "");
      setJs(row.custom_js ?? "");
    }
  }, [row?.id]);
  if (!row)
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-[var(--color-ink-soft)]">
        يُحفظ في قاعدة البيانات فقط. لتنفيذه في الصفحة، يقرأه المُصيّر (Renderer). الكود الديناميكي
        لا يُحقن تلقائياً في التطبيق الحالي؛ استخدم Custom HTML عبر مكوّن "custom_html".
      </p>
      {[
      ...
      <button
        onClick={async () => {
          ...
          await upsert({
            data: {
              table: "website_settings" as any,
              values: { id: row.id, custom_html: html, custom_css: css, custom_js: js },
            },
          });
          qc.invalidateQueries({ queryKey: ["wb", "website_settings"] });
          setOk("تم الحفظ");
...
function ImportExportPanel() {
  ...
  const doExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `website-builder-${Date.now()}.json`;
      a.click();
...
function BackupsPanel() {
  ...
  const qc = useQueryClient();
  const q = useQuery({
    queryKey: ["wb", "website_backups"],
    queryFn: () => list({ data: { table: "website_backups" as any } }),
  });
    ...
    <div>
      <div className="bg-[var(--color-surface)] rounded-xl p-4 mb-4 flex gap-2 items-end">
        ...
        <button
          ...
          className="btn-primary"
        >
          {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Plus className="w-4 h-4" />} نسخة
          احتياطية
        </button>
      </div>
      ...
      <div className="space-y-2">
        {(q.data ?? []).map((b: any) => (
          <div
            key={b.id}
            className="bg-[var(--color-surface)] rounded-lg p-3 flex justify-between items-center gap-2"
          >
            <div className="min-w-0 flex-1">
            ...
            <div className="flex gap-2">
              <button
                onClick={async () => {
                  if (
                    !confirm("استعادة هذه النسخة ستستبدل بيانات Website Builder الحالية. متابعة؟")
                  )
                    return;
