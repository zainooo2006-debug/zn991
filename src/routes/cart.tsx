      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
            ...
            <li key={i.id} className="card-clean p-3 flex gap-3 items-center">
              {i.image ? (
                <img
                  src={i.image}
                  alt={i.name}
                  className="w-20 h-20 rounded-lg object-cover bg-[var(--color-surface)]"
                />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[var(--color-gold)] font-black mt-1">
                  {(i.price * i.qty).toLocaleString()} ر.ي
                </p>
                <div className="flex items-center gap-2 mt-2">
                  <button
                    onClick={() => setQty(i.id, i.qty - 1)}
                    className="w-7 h-7 rounded-full border border-[var(--color-hairline)] flex items-center justify-center"
                    aria-label="نقص"
                  >
                  ...
                  <button
                    onClick={() => remove(i.id)}
                    className="mr-auto p-1.5 text-red-600"
                    aria-label="حذف"
                  >
        ...
        <div className="card-clean p-5 mt-6 sticky bottom-20 md:static">
          <div className="flex items-center justify-between text-lg">
            ...
            <span className="text-[var(--color-gold)] font-black text-2xl">
          ...
          <Link to="/shop" className="btn-outline w-full mt-2 justify-center">
            إضافة المزيد
          </Link>
        </div>
      </div>
